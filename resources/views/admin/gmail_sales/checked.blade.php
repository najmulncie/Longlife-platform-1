...
@section('content')
<h1 class="text-2xl font-semibold mb-4">Checked Gmail Sales</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-blue-100">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-sm font-medium text-blue-700">User</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-blue-700">Gmail</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-blue-700">Password</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-blue-700">Status</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-blue-700">Submitted</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-blue-700">Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-blue-200">
                    @foreach($gmailSales as $sale)
                        <tr class="hover:bg-blue-50">
                            <td class="px-4 py-2">
                                <input type="checkbox" name="ids[]" value="{{ $sale->id }}" class="form-checkbox text-blue-600">
                            </td>
                            <td class="px-4 py-2 text-sm text-blue-800">{{ $sale->user->name ?? 'N/A' }}</td>
                            <td class="px-4 py-2 text-sm text-blue-800">{{ $sale->gmail }}</td>
                            <td class="px-4 py-2 text-sm text-blue-800">{{ $sale->gmail_password }}</td>
                            <td class="px-4 py-2 text-sm text-blue-800">
                                <span class="inline-block px-2 py-1 text-xs rounded-lg bg-blue-100 text-blue-700">
                                    {{ ucfirst($sale->status) }}
                                </span>
                            </td>
                            <td class="px-4 py-2 text-sm text-blue-800">{{ $sale->created_at->diffForHumans() }}</td>
                            <td class="px-4 py-2 text-sm text-blue-800">
